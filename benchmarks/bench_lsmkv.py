...
async def benchmark_compaction_throughput(num_keys: int = 50000) -> dict:
    ...
    with TemporaryDirectory() as tmp_dir:
        engine = StorageEngine(
            data_dir=tmp_dir,
            memtable_size_bytes=4 * 1024 * 1024,  # Smaller memtable to trigger compaction
...
async def run_all_benchmarks():
    ...
    print(f"  Compaction throughput: {compaction_results['compaction_throughput_keys_per_sec']:,.0f} keys/sec")
    print(f"  Compaction time: {compaction_results['compaction_time_sec']:.2f} sec")
    
    print("\n" + "=" * 70)
    print("Summary")
    print("=" * 70)
    print(f"SET throughput (100k):  {set_results['throughput_keys_per_sec']:>12,.0f} keys/sec")
    print(f"GET throughput:         {get_results['throughput_keys_per_sec']:>12,.0f} keys/sec")
    ...
    print(f"Compaction throughput:  {compaction_results['compaction_throughput_keys_per_sec']:>12,.0f} keys/sec")
